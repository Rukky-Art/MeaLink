...
const PartnerDashboard = () => {
  ...
  const filteredFood = useMemo(() => {
    ...
    if (activeFilter !== 'all') {
      list = list.filter((f) => {
        const isMine = myClaims.some(
          (c) => c.food === f.id || c.food?.id === f.id
        );
...
const FoodRow = ({ food, isMyClaim, onClick, index }) => {
  ...
  const status = isMyClaim
    ? 'Claimed by you'
    : isClaimedByOthers
      ? 'Taken'
      : 'Available';

  const statusType = isMyClaim ? 'mine' : isClaimedByOthers ? 'taken' : 'available';
