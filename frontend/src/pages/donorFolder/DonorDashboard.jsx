...
//   const { myListings = [], loading } = useSelector((state) => state.donor);
...
import {
  ...
  selectTotalMealsDistributed,
  selectMonthlyDistributedMealsData,
...
const DonorDashboard = () => {
  ...
  const isLoading = useSelector(selectDonorIsLoading);
  const claimedListings = useSelector(selectClaimedListings);
  const availableListings = useSelector((state) => selectListingsByStatus(state, 'available'));
  const totalMealsDistributed = useSelector(selectTotalMealsDistributed);
  const monthlyData = useSelector(selectMonthlyDistributedMealsData);
  ...
  const stats = {
    ...
    unclaimed: availableListings.length,
    meals: totalMealsDistributed,
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {isLoading ? (
          ...
          <>
            ...
            <StatCard title="Successfully Claimed" value={stats.claimed} />
            <StatCard title="Meals Redistributed" value={stats.meals} />
            <StatCard title="Unclaimed" value={stats.unclaimed} />
          </>
        )}
      </div>
