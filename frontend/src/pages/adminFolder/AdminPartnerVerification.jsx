...
const AdminPartnerVerification = () => {
  ...
  useEffect(() => {
    dispatch(fetchPanelUsers({ role: 'partner', status: activeTab }));
  }, [dispatch, activeTab]);
  ...
  const handleApprove = async () => {
    const result = await dispatch(reviewPanelUser({
      userId: selected.id, isVerified: true, adminNote,
    }));
    if (reviewPanelUser.fulfilled.match(result)) {
      closeModal();
      showToast(`${selected.name} has been verified`);
    }
  };
  ...
  const handleReject = async () => {
    ...
    const result = await dispatch(reviewPanelUser({
      userId: selected.id, isVerified: false, rejectionReason, adminNote,
    }));
    if (reviewPanelUser.fulfilled.match(result)) {
      closeModal();
      showToast(`${selected.name} has been rejected`, 'error');
    }
  };
        ...
        <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl font-black text-slate-900">Partner Verification</h1>
            ...
            <p className="text-sm text-slate-400">
              Review partner organisations — verify their business registration before they can post food listings.
            </p>
          </div>
          ...
          <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-slate-50 rounded-xl border border-slate-200 text-sm outline-none focus:border-purple-300 transition-all" />
            </div>
          </div>
          ...
          {pageLoading ? (
            ...
            <div className="p-16 text-center">
              <p className="text-red-500 font-bold mb-2">
                {typeof pageError === 'string' ? pageError : pageError?.detail || 'Failed to load partners.'}
              </p>
              <button onClick={() => dispatch(fetchPanelUsers({ role: 'partner', status: activeTab }))} className="text-brand-green font-bold underline text-sm">
                Retry
              </button>
            </div>
          ) : (
                  ...
                  {filtered.length === 0 ? (
                    <tr><td colSpan={6} className="px-5 py-16 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm font-bold text-slate-400">No partner organisations found</p>
                      </div>
                    </td></tr>
                  ) : filtered.map(d => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-7 py-6 space-y-6">
              ...
              {/* Distribution details — partner specific */}
              {(selected.distribution_venue || selected.distribution_capacity || selected.container_type) && (
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Distribution Details</p>
                  <div className="bg-purple-50 rounded-2xl px-4 divide-y divide-purple-100">
                    {selected.distribution_venue    && <DetailRow label="Venue / Hall" value={selected.distribution_venue} />}
                    {selected.distribution_capacity && <DetailRow label="Capacity"     value={`${selected.distribution_capacity} people`} />}
                    {selected.container_type        && <DetailRow label="Containers"   value={selected.container_type} />}
                  </div>
                </div>
              )}
              ...
              {modalMode === 'approve' && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-100 rounded-2xl p-4">
                    ...
                    <div>
                      <p className="text-sm font-bold text-emerald-800">Approve this partner</p>
              ...
              {modalMode === 'reject' && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-2xl p-4">
                    ...
                    <div>
                      <p className="text-sm font-bold text-red-800">Reject this partner</p>
